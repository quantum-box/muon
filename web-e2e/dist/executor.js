        ...
        const run = async () => {
            ...
            try {
                ...
                if (resolved.optional) {
                    ctx.log?.(`optional step failed, continuing: ${describeStep(resolved)}: ${error instanceof Error ? error.message : String(error)}`);
                    return;
                }
                throw error;
            }
...
function deepInterpolateSafe(step, vars) {
    try {
...
async function executeStep(step, ctx) {
    ...
    switch (step.type) {
        case 'launchApp': {
            ...
            const url = step.url ??
                (ctx.config.url !== undefined
                    ...
                    : undefined);
            if (!url) {
                throw new FlowError('launchApp needs a "url" in the flow config header or step params', ctx.filePath);
            }
        ...
        case 'assertNotVisible':
            ...
            return;
        case 'assertTitle':
            await pollText(() => page.title(), step.pattern, step.timeout ?? DEFAULT_ASSERT_TIMEOUT, 'page title', ctx);
            return;
        case 'assertUrl':
            await pollText(() => Promise.resolve(page.url()), step.pattern, step.timeout ?? DEFAULT_ASSERT_TIMEOUT, 'page URL', ctx);
            return;
        case 'scroll': {
        ...
        case 'runFlow': {
            if (ctx.depth >= MAX_FLOW_DEPTH) {
                throw new FlowError(`runFlow nesting exceeded ${MAX_FLOW_DEPTH} levels (cycle?)`, ctx.filePath);
            }
            const baseDir = ctx.filePath === '<inline>' ? process.cwd() : path.dirname(ctx.filePath);
...
async function pollText(get, pattern, timeout, what, ctx) {
    const matcher = toTextMatch(pattern);
    const deadline = Date.now() + timeout;
    let last = '';
    for (;;) {
        last = await get();
        const ok = typeof matcher === 'string' ? last.includes(matcher) : matcher.test(last);
        if (ok)
            return;
        if (Date.now() >= deadline) {
            throw new FlowError(`${what} ${JSON.stringify(last)} did not match ${pattern} within ${timeout}ms`, ctx.filePath);
        }
...
export function describeStep(step) {
    ...
    switch (step.type) {
        ...
        case 'inputText':
            return `inputText ${JSON.stringify(step.text)}${step.selector ? ` into ${describeSelector(step.selector)}` : ''}`;
        case 'pressKey':
